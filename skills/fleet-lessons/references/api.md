...
| Receipt keys | `lessons.receipt_jwks` | `GET /.well-known/jwks.json` |
...
`idempotency_key`. Include `run_binding` only when Fleet supplied one for the
authorized run. Unknown fields are errors.
...
the internal signed credential is never returned to the harness.
